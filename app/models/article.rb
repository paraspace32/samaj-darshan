class Article < ApplicationRecord
  ...
  has_one_attached :cover_image do |attachable|
    attachable.variant :hero,     resize_to_fill: [1200, 600], format: :webp, saver: { quality: 80 }
    attachable.variant :card,     resize_to_fill: [600, 340],  format: :webp, saver: { quality: 75 }
    attachable.variant :thumb,    resize_to_fill: [120, 80],   format: :webp, saver: { quality: 70 }
    attachable.variant :carousel, resize_to_limit: [1200, 800], format: :webp, saver: { quality: 80 }
  end

  has_many_attached :images do |attachable|
    attachable.variant :carousel, resize_to_limit: [1200, 800], format: :webp, saver: { quality: 80 }
    attachable.variant :thumb,    resize_to_fill: [200, 200],   format: :webp, saver: { quality: 70 }
  end

  enum :status, { draft: 0, pending_review: 1, approved: 2, published: 3, rejected: 4 }
  enum :article_type, { news: 0, magazine: 1 }, prefix: :type
